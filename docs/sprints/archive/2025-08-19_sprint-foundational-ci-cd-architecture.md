...
## Step 5: Foundational UI Layout and Canvas Setup

**Intent:** To establish the main application layout and integrate a basic PixiJS canvas, preparing the ground for future interactive features. This step pivots from database-driven content to a file-based approach using the TIFF format as the ultimate goal for data persistence.

- [x] **Establish Main Application Layout**:
  - **Action**: Created a responsive three-column layout (`ThreeColumnLayout.tsx`) that serves as the primary structure for the application's UI. This component defines the left, center, and right panels.
  - **Reasoning**: This provides a consistent and scalable structure for organizing UI components and tools, ensuring a predictable user experience across different features.

- [x] **Integrate Basic PixiJS Canvas**:
  - **Action**: Set up a `WorkspaceRenderer.tsx` component to encapsulate the PixiJS canvas. This component initializes a basic PixiJS application and renders it within the center column of the main layout.
  - **Reasoning**: This establishes the core rendering surface for the comic creation tools. Encapsulating the canvas logic in a dedicated component promotes modularity and separates rendering concerns from the main application logic.

- [x] **Future Data Storage Strategy**:
  - **Action**: Decided to move away from using a database for storing canvas content coordinates and other graphical data.
  - **New Direction**: The application will now target the TIFF (Tagged Image File Format) as the primary data store for all canvas-related artwork and metadata.
  - **Reasoning**: The TIFF format is a powerful and extensible standard in the graphics industry. It can store high-quality raster images, vector paths, layers, and custom metadata within a single file. This approach simplifies data management, improves portability, and aligns better with professional graphics software workflows compared to storing scattered data points in a database.
...
- [ ] **Create `main-branch.yml` Workflow**:
  ...
  - **Reasoning**: This fully automates the release process. It ensures that every release is built in a clean, consistent environment for multiple platforms and a professionall signed and easily accessible to users.
